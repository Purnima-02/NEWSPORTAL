...
async def async_task2(request):
    ...
    return JsonResponse({"status": "success12345600"})
# @api_view(['POST'])
...
def db_data(username, password):
    # Ensure synchronous ORM queries are encapsulated in this function
    return list(AdminEmployeeCredentials.objects.filter(name=username, password=password).values())
...
async def validateAdminEmployee(request):
    try:
        ...
        # Properly await the result of the database call
        user = await db_data(username, password)

        if user:
            return JsonResponse({"data": user}, status=200)
        else:
            return JsonResponse({"data": "Invalid user"}, status=404)
